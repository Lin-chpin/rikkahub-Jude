...
object ModelRegistry {
    ...
    private val GPT_5_6_TERRA = defineModel {
        exact("gpt-5.6-Terra")
        visionInput()
        toolReasoningAbility()
    }

    private val GPT_5_6_LUNA = defineModel {
        exact("gpt-5.6-Luna")
        visionInput()
        toolReasoningAbility()
    }

    private val GPT_5_6_SOL = defineModel {
        exact("gpt-5.6-Sol")
        visionInput()
        toolReasoningAbility()
    }
    ...
    private val DEEPSEEK_V4_OR_LATER = defineModel {
        tokens(
            tokenRegex("deepseek"),
            tokenRegex("v"),
            tokenRegex("(?:[4-9]|[1-9][0-9]+)")
        )
        toolReasoningAbility()
    }

    private val DEEPSEEK_FLASH = defineModel {
        tokens(tokenRegex("deepseek"), tokenRegex("flash"))
        toolReasoningAbility()
    }

    val DEEPSEEK_RESPONSES = defineGroup {
        add(DEEPSEEK_V4_OR_LATER, DEEPSEEK_FLASH)
    }
